    ...
    <section class="px-4 md:px-14">
      <div class="max-w-[1180px] mx-auto relative">
        ...
        <RouterLink to="/search"
          ...
          <span class="hidden sm:inline">{{ t('city.allCities') }}</span>
        </RouterLink>
        ...
        <div class="absolute left-4 md:left-7 bottom-4 md:bottom-7 right-4 md:right-7 flex items-end justify-between gap-3 pointer-events-none text-white">
          ...
          <div class="hidden md:flex items-center gap-3.5 pointer-events-auto flex-shrink-0 bg-white/[0.96] rounded-[18px] px-[18px] py-[14px]">
            ...
            <div class="whitespace-nowrap">
              <div class="text-xs text-wt-sub font-semibold">{{ t('city.welcomeScore') }}</div>
              ...
              <div class="text-xs text-wt-sub">
                ...
                {{ t('city.reviews', city.score_count) }}
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="max-w-[1180px] mx-auto px-4 md:px-14 pt-6 pb-16 grid grid-cols-1 md:grid-cols-[1fr_372px] gap-10 md:gap-12 items-start">
      ...
      <main>
        ...
        <p class="text-[17px] md:text-[18.5px] leading-relaxed text-wt-ink mb-3">
          <strong>{{ t('city.isItSafe', { city: city.name }) }}</strong>
          {{ city.description }}
        </p>
        <p class="text-[15px] leading-relaxed text-wt-sub mb-8">
          {{ t('city.scoreBlend', { score: numScore.toFixed(1), count: city.score_count, city: city.name }) }}
        </p>
        ...
        <div class="md:hidden mb-8">
          ...
          <p class="text-center mt-3 text-xs text-wt-faint">
            ...
            {{ t('city.everyHostVetted') }}
          </p>
        </div>

        <!-- score breakdown -->
        <section class="mb-10">
          <div class="font-mono text-[11px] tracking-[1.2px] text-wt-coral uppercase mb-2">{{ t('city.breakdownLabel') }}</div>
          <h2 class="font-serif text-2xl md:text-[27px] font-semibold m-0 mb-4 tracking-[-0.4px]">{{ t('city.howWelcoming', { city: city.name }) }}</h2>
          <div class="bg-white rounded-2xl p-5 md:p-6 shadow-sm flex flex-col sm:flex-row items-center sm:items-start gap-5">
            ...
            <div class="flex flex-col gap-3.5 w-full">
              <div class="flex items-center gap-2.5">
                <span class="w-16 md:w-24 text-[13px] text-wt-sub font-medium">{{ t('city.safety') }}</span>
        ...
        <section class="mb-10">
          <div class="font-mono text-[11px] tracking-[1.2px] text-wt-coral uppercase mb-2">{{ t('city.realReviewsLabel') }}</div>
          <h2 class="font-serif text-2xl md:text-[27px] font-semibold m-0 mb-4 tracking-[-0.4px]">{{ t('city.whatTravellersSay') }}</h2>
        ...
        <section v-if="city.places?.length" class="mb-10">
          <div class="font-mono text-[11px] tracking-[1.2px] text-wt-coral uppercase mb-2">{{ t('city.seeItLabel') }}</div>
          <h2 class="font-serif text-2xl md:text-[27px] font-semibold m-0 mb-4 tracking-[-0.4px]">{{ t('city.placesIn', { city: city.name }) }}</h2>
...
function load() {
  store.fetchCity(route.params.slug as string)
}

onMounted(load)

// Re-fetch on locale change so the description/meta_description come back
